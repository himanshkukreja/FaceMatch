...
def generate_data_pickle_file(config_path, params_path):
    ...
    logging.info(f"Total actors read: {len(actors)}")
    logging.info(f"Total actors images read: {len(filename)}")
    pickle.dump(filename,open(pickle_file,'wb'))
    logging.info(f"Pickle file created, Path: {pickle_file}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.info("Stage_01_completed>>>>>\n")
